...
def model_coeffs_from_iv_coeffs_linear(
    iv_coeffs,
    loading_norminfo=None,
    intercept_norminfo=None,
    coeff_sum_value=None,
    trans_intercept_value=None,
):

    return general_model_coeffs_from_iv_coeffs(
        iv_coeffs=iv_coeffs,
        iv_intercept_position=-1,
        has_trans_intercept=False,
        loading_norminfo=loading_norminfo,
        intercept_norminfo=intercept_norminfo,
        coeff_sum_value=coeff_sum_value,
...
def coeff_names_linear_with_constant(included_factors, params_type, factor, stage):
    names = coeff_names_linear(included_factors, params_type, factor, stage)
...
def coeff_names_log_ces(included_factors, params_type, factor, stage):
    ...
    names = [fs1.format(stage, factor, i_fac) for i_fac in included_factors[:-1]]
    if params_type == "long":
...
def coeff_names_translog(included_factors, params_type, factor, stage):
    names = [
        "translog__{}__{}__{}".format(stage, factor, i_fac)
    ...
    for i, i_fac1 in enumerate(included_factors):
        for i_fac2 in included_factors[i:]:
            if i_fac1 == i_fac2:
                names.append(
                    "translog__{}__{}__{}-squared".format(stage, factor, i_fac1)
                )
            else:
                names.append(
                    "translog__{}__{}__{}-{}".format(stage, factor, i_fac1, i_fac2)
                )
...
def coeff_names_no_squares_translog(included_factors, params_type, factor, stage):
    names = [
        "translog__{}__{}__{}".format(stage, factor, i_fac)
        for i_fac in included_factors
    ]
...
def general_model_coeffs_from_iv_coeffs(
    ...
    meas_coeffs = pd.DataFrame(
        data=0, index=iv_coeffs.index, columns=["loadings", "intercepts"]
    )
    ...
    if has_trans_intercept is False:
        ...
        trans_intercept_value = (
            intercept_coeff - intercept_norm_val
        ) / corresponding_loading
    ...
    return (
        meas_coeffs,
        gammas,
        newly_identified_coeff_sum_value,
