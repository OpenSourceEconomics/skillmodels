...
def simulate_datasets(factor_names,control_names,meas_names,means,covs,weights, nobs, nt,
                      transition_names, transition_argument_dicts,shock_variances,loadings,deltas,meas_variances):
    ...
    Args:
         nt (int): number of time periods the dataset contains
    ...
    out = np.zeros((nt,nobs,nfac+ncont+nmeas+2))
    ...
    out[:,:,(nfac+2):(nfac+ncont+2)] = generate_start_factors_and_control_variables_v3(
                                   means, covs, weights, nobs, factor_names,
                                   control_names)[1]
    #out[0,:,(nfac+ncont+1):] = measurements_from_factors(out[0,:,1:(nfac+1)],
                                             #  out[0,:,(nfac+1):(nfac+ncont+1)],
                                             ##  loadings, deltas,meas_variances, 
                                              # measurement_names)
    # Generate next period factors, controls and meas from them
    for i in range(1,nt):
    ...
    out[:,:,(nfac+ncont+2):] = measurements_from_factors(out[:,:,2:(nfac+2)].reshape(nobs*nt,nfac),
                                              out[:,:,(nfac+2):(nfac+ncont+2)].reshape(nobs*nt,ncont),
                                              loadings, deltas,meas_variances, 
                                              meas_names).values.reshape(nt,nobs,nmeas)
    out[:,:,0] = np.repeat(range(nt),nobs).reshape(nt,nobs)
    out[:,:,1] = np.array(range(nobs))
    
    observed_data = pd.DataFrame(
                         data = np.concatenate(
                                 [out[:,:,0:2],out[:,:,(nfac+ncont+1):],out[:,:,(nfac+1):(nfac+ncont+1)]],
                                        axis = 2).reshape(nobs*nt,2+nmeas+ncont),
                        columns = ['period_t','child_id'] + meas_names + control_names
                        )
                         
    latent_data = pd.DataFrame(
                         data = out[:,:,0:(nfac+2)].reshape(nobs*nt,2+nmeas+ncont),
