...
def _mv_student_t(mu, cov, nu):
    ...
    Notes:
        ...
        Can be extended to generate samples of any size:
            https://bit.ly/2tHQAP1
    """
    d = len(cov)
    u = np.sqrt(np.random.chisquare(nu)/nu)
    y = multivariate_normal(np.zeros(d),cov*(nu-2)/nu)
    mv_t = mu + y/u
    return mv_t
