...
def add_missings(data, meas_names, share, serial_corr=0.0, within_period_corr=0.0):
    ...
    # 2.
    p = share * np.size(data.values) / np.size(data[meas_names].values)
    ...
    while replaced_share < share:
        for i in set(
            data_with_missings.index
        ):  # alternatively randomly choose individual??
            ind_data = data_with_missings.loc[i]

            if binomial(1, p) == 1:
                ind_data.iloc[0][meas_names[0]] = np.nan
            for t in range(1, len(ind_data)):
                if np.isnan(ind_data.iloc[t - 1][meas_names[0]]):
                    if binomial(1, p_t_11) == 1:
                        ind_data.iloc[t][meas_names[0]] = np.nan
                else:
                    if binomial(1, p_t_10) == 1:
                        ind_data.iloc[t][meas_names[0]] = np.nan
                for m in range(1, len(meas_names)):
                    if np.isnan(ind_data.iloc[t][meas_names[m - 1]]):
                        if binomial(1, p_m_11) == 1:
                            ind_data.iloc[t][meas_names[m]] = np.nan
                    else:
