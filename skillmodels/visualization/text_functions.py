...
def items_to_text_enumeration(items, name=None):
    ...
    assert len(items) >= 1, (
        'Empty list encountered in list_to_text')
...
def title_text(basic_name, factors=None, periods=None, stages=None, wrap_width=None):
    assert periods is None or stages is None, (
        'You cannot specify periods and stages.')
...
def write_figure_tex_snippet(figure_path, title, width=None, height=None):
    ...
    include = r'\includegraphics[width={}\textwidth,height={}\textheight,keepaspectratio]{{{}}}' + newline
