...
from skillmodels.estimation.parse_params import (
    _map_params_to_delta,
    _map_params_to_h,
    _map_params_to_r,
    _map_params_to_q,
    _map_params_to_x,
...
def test_map_params_to_q():
    ...
    expected = np.vstack(
        [np.diag([2., 4, 6]), np.diag([8., 10, 12])]).reshape((2, 3, 3))
...
def test_map_params_to_p_square_root_filters(p_params):
    ...
    expected0 = np.linalg.cholesky(
        np.array([[1, 0.1, 0.2], [0.1, 2, 0.3], [0.2, 0.3, 3]])).T
    expected1 = np.linalg.cholesky(
        np.array([[4, 0.5, 0.6], [0.5, 5, 0.7], [0.6, 0.7, 6]])).T
...
def test_map_params_to_trans_coeffs():
    ...
    expected = [
        np.array([[0, 1, 2], [7, 8, 9.]]),
        np.array([[3.], [10]]),
        np.array([[4., 5, 6], [11, 12, 13]])
    ]
